...
//     gStyle->SetPadRightMargin(0.05);
    ...
    std::cout<<"NO PROBLEMA CON IL BINNING"<<std::endl;
...
// 	     treeMC->SetBranchAddress("lep_TuneP_numberOfValidMuonHits",lep_TuneP_numberOfValidMuonHits);	
     ...
     //treeMC->SetBranchAddress("gen_lep_pt",gen_lep_pt); 
		...
		phi_MC_2016_clear[j] = new TH1F(samples[j] + "_Lepton_Phi_2016", samples[j] + "_Lepton_Phi_2016", binnum_phi, PHI_BINS);
         dimuon_BB_MC_clear = new TH1F("MC_dimuon_BB", "MC_dimuon_BB", 50, ciao_bhu);//NMBINS, logMbins);         
         dimuon_BE_MC_clear = new TH1F("MC_dimuon_BE", "MC_dimuon_BE", 50, ciao_bhu);//NMBINS, logMbins);         
         dimuon_cumulative_MC_clear = new TH1F("MC_dimuon_cumulative", "MC_dimuon_cumulative", 50, ciao_bhu);//NMBINS, logMbins);                         
         dimuon_cumulative_BB_MC_clear = new TH1F("MC_dimuon_cumulative_BB", "MC_dimuon_cumulative_BB", 50, ciao_bhu);//NMBINS, logMbins);         
         dimuon_cumulative_BE_MC_clear = new TH1F("MC_dimuon_cumulative_BE", "MC_dimuon_cumulative_BE", 50, ciao_bhu);//NMBINS, logMbins);         		
...
// 				}
					...
					if(fabs(lep_eta[0]) < 1.2 && fabs(lep_eta[1]) < 1.2)
						dimuon_BB_MC_clear->Fill(dil_mass,  weight_2018_BB[j]);
					else
						dimuon_BE_MC_clear->Fill(dil_mass,  weight_2018_BE[j]);
					...
					for(int h = 0; h < 2; h++){ // for on two muons in the event									    						
						...
						if(fabs(lep_eta[h]) < 1.2)
							pt_MC_BB_clear->Fill(lep_pt[h],  weight_2018_BB[j]);
						else
							pt_MC_BE_clear->Fill(lep_pt[h],  weight_2018_BE[j]);
