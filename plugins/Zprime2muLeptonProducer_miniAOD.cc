...
//  float mupt = GeneralizedEndpoint().GeneralizedEndpointPt(newTrack->pt(),newTrack->charge(),newTrack->eta(),phi,123,1);  // for smear resolution in 2017

// GEScaleSyst *GE = new GEScaleSyst();
// GE->SetVerbose(1);
// float mupt = GE->GEScaleCorrPt(-1, newTrack->pt(), newTrack->eta(), newTrack->phi(), newTrack->charge());
