...
void Nminus1plots(){
    ...
    /*  float sigma[mc] = {
     ...
     2.886,          //2    dy200to400
     0.2517,         //3    dy400to800
     0.01707,        //4    dy800to1400
     1.366E-3,       //5    dy1400to2300
     8.178E-5,       //6    dy2300to3500
     3.191E-6,       //7    dy3500to4500
     2.787E-7,       //8    dy4500to6000
     9.569E-9,       //9    dy6000toInf
     88.29,      //10    ttbar
     34.91,      //11    tW
     34.97,      //12    Wantitop
     75.8,       //13    WW
    ...
    const double LUMINOSITY = 61298.77523; //in pb
    ...
    TH1F* DY_weights_all_nw = new TH1F("DY_weights_all_nw ", "DY_weights_all_nw", 100, -1.0E-10, 100);
    DY_weights_all_nw->Sumw2();
    TH1F* DY_weights_Zprime_nw = new TH1F("DY_weights_Zprime_nw ", "DY_weights_Zprime_nw", 100, -1.0E-10, 100);
    DY_weights_Zprime_nw->Sumw2();
    TH1F* DY_weights_all_w = new TH1F("DY_weights_all_w ", "DY_weights_all_w", 100, -1.0E-10, 100);
    DY_weights_all_w->Sumw2();
    TH1F* DY_weights_Zprime_w = new TH1F("DY_weights_Zprime_w ", "DY_weights_Zprime_w", 100, -1.0E-10, 100);
    DY_weights_Zprime_w->Sumw2();
    ...
    for(int j=0; j<mc; j++){
        ...
        for ( int p=0; p<ne ;p++){
            ...
            if (j==0 || j==1 || j==2 || j==3 || j==4 || j==5 || j==6 || j==7 || j==8 || j==9) {
                ...
                DY_weights_all_nw->Fill(weight[j]*genWeight);
                nw[j] = nw[j] + 1;
                DY_weights_all_w->Fill(weight[j]*genWeight, weight[j]*genWeight);
                w[j] = w[j] + weight[j]*genWeight;
            }
            ...
            if(
               ...
               vertex_chi2 < 20
                ...
                if(j==10){ //for ttbar
                    ...
                    DY_weights_Zprime_nw->Fill(weight[j]);
                    Znw[j] = Znw[j] + 1;
                    
                    DY_weights_Zprime_w->Fill(weight[j], weight[j]);
                    Zw[j] = Zw[j] + weight[j]*genWeight;
    ...
    for(int j=0; j<10; j++){
        std::cout<<nw[j]<<"  "<<w[j]<<" "<<Znw[j]<<"  "<<Zw[j]<<std::endl;
    }
